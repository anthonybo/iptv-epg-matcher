...
/**
 * Forwards local notification state to the global ToastContainer so
 * all status feedback in the app uses the same broadcast-monitor
 * chyron treatment instead of two separate inline implementations.
 * Each notification object triggers one showToast(...) call; the
 * onConsumed callback resets the local state so the next notification
 * (potentially identical content) fires again cleanly.
 *
 * Duration mapping: warnings/errors stay 5s, info 4s, success 3s —
 * matches the legacy inline behaviour callers expect.
 */
function NotificationBridge({ notification, onConsumed }) {
  React.useEffect(() => {
    if (!notification) return;
    const durationMs =
      notification.type === 'error' ? 6000
      : notification.type === 'warning' ? 5000
      : notification.type === 'info' ? 4000
      : 3000;
    showToast(
    ...
    onConsumed();
    // We want this to refire whenever a new notification object lands.
...
const MyIPTVs = ({
  ...
  return (
    <div className="space-y-6">
      {/* Notification routed through the global ToastContainer for a
          single source of truth. Renders nothing locally — the
          observer in ToastContainer listens to the global 'showToast'
          event and the broadcast-monitor chyron styling lives there.
          This effect fires when setNotification(...) is called from
          any of the handlers and forwards to the container. */}
      <NotificationBridge notification={notification} onConsumed={() => setNotification(null)} />
