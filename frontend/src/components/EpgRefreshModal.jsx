...
const EpgRefreshModal = ({ isOpen, onClose, progress }) => {
  ...
  const { sources = [], currentSource, totalSources, status, error, currentMessage, progressInfo } = progress;
  ...
  if (isMinimized) {
    return (
      <div
        ...
        className="fixed bottom-6 left-6 z-50 bg-white rounded-lg shadow-2xl border border-gray-200 cursor-pointer hover:shadow-xl transition-shadow"
        ...
        <div className="flex-1 overflow-y-auto p-6 min-h-0">
          ...
          {totalSources > 0 && (
            <div className="mb-5">
              <div className="flex justify-between text-sm font-medium text-gray-700 mb-2">
                ...
                <span className="text-blue-600">{currentSource} / {totalSources} sources</span>
              </div>
          ...
          {status === 'starting' && (
            <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg mb-4">
              ...
              <span className="text-blue-900">Starting EPG refresh...</span>
            </div>
          )}
          ...
          {status === 'refreshing' && (
            <div className="flex items-start gap-3 p-4 bg-blue-50 rounded-lg mb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {progressInfo && (
                  <div className="text-blue-600 text-sm mt-1.5 font-semibold">
                    {progressInfo}
                  </div>
                )}
                {currentMessage && (
                  <div className="text-blue-700 text-xs mt-2 font-mono break-all leading-relaxed bg-blue-100/50 p-2 rounded">
                    {currentMessage}
                  </div>
                )}
              </div>
          ...
          {status === 'polling' && (
            <div className="flex items-center gap-3 p-4 bg-yellow-50 rounded-lg mb-4">
              ...
              <div className="flex-1">
                <div className="text-yellow-900 font-medium">Python EPG parser is running...</div>
                <div className="text-yellow-700 text-xs mt-1">Downloading and processing EPG sources (this may take 5-15 minutes for large files)</div>
              </div>
            </div>
          )}
          ...
          {status === 'complete' && !error && (
            <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg mb-4">
              ...
              <span className="text-green-900">EPG sources refreshed successfully!</span>
            </div>
          )}
          ...
          {sources.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-gray-700 mb-3">Sources ({sources.length})</h3>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                {sources.map((source, index) => (
                  ...
                  >
                    ...
                    {source.status === 'error' && source.enabled !== false && (
                      <svg className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    )}
                    {source.status === 'pending' && source.enabled !== false && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {source.url && (
                        <div className="text-xs text-gray-500 break-all mt-1 leading-relaxed">
                          {source.url}
                        </div>
                      )}
                      {source.notes && (
                      ...
                      {source.error && (
                        <div className="text-xs text-red-700 mt-1.5 break-words">
                          {source.error}
                        </div>
                      )}
                    </div>
