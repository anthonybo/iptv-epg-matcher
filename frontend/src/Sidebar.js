...
const TabButton = ({ id, label, icon, onClick, isActive, count }) => (
  <button
    type="button"
    onClick={() => onClick(id)}
    className={`group flex w-full items-center justify-between rounded-xl px-4 py-2.5 text-sm transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:ring-offset-2 focus:ring-offset-slate-950 ${
      isActive ? 'bg-blue-500/20 text-blue-100' : 'text-slate-300 hover:bg-slate-800/80'
    }`}
  >
    <span className="flex items-center gap-3">
      <span className={`flex h-9 w-9 items-center justify-center rounded-lg border transition-colors duration-150 ${
    ...
    <aside className={`${visibilityClass} sticky top-0 h-screen w-72 flex-col border-r border-slate-800/80 bg-slate-950/80 text-slate-200 backdrop-blur`}> 
      <div className="border-b border-slate-800/70 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.35em] text-blue-400/70">IPTV</p>
            <h2 className="text-lg font-semibold text-slate-100">EPG Matcher</h2>
          </div>
        </div>
      </div>

      <nav className="flex-1 overflow-y-auto px-4 py-6">
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Navigation</p>
      ...
      {totalChannels > 0 && (
        <div className="space-y-4 border-t border-slate-800/70 px-5 py-5">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Statistics</p>
          ...
          <button
            type="button"
            onClick={handleReset}
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 px-4 py-2 text-sm font-semibold text-slate-200 transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:ring-offset-2 focus:ring-offset-slate-950"
          >
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M2.5 2v6h6" />
